...
Open **Network → Radios** to inspect the radios attached to this BBS. Each panel identifies the network and local slot, connection state, channel names, contacts or known nodes, and supported identity controls. The old `/system/meshtastic` address still opens Radios.
