...
def home_network(node_id) -> str:
    ...
    Returns 'meshtastic', 'meshcore', 'mqtt', 'emulator', or 'ssh'.
    ...
    MeshCore node by the unrecognised-shape default below. Authenticated SSH
    sessions similarly use server-derived 'ssh:<account_id>' identities.
    """
    text = str(node_id or '').strip()
    if text.startswith('!'):
        return 'meshtastic'
...
def send_account_unlink_to_bbs_nodes(node_id, unlinked_at, bbs_nodes, interface):
    """Advertise that a device was taken off its account.

    Its own frame, and its own capability: a peer that only knows about
    links would otherwise keep re-advertising the one this removes.
    """
    try:
        from db_operations import peer_supports
    except Exception:
        return 0
    message = "ACCTUNLINK|{0}|{1}".format(str(node_id), str(unlinked_at))
    sent = 0
    for peer_id in bbs_nodes or []:
        if peer_supports(peer_id, 'acct') and peer_supports(peer_id, 'acctmv'):
            _send_one_sync(message, peer_id, interface)
            sent += 1
    return sent


def send_node_role_to_bbs_nodes(node_id, role, updated_at, bbs_nodes, interface):
    """Advertise one node's role to peers that understand roles.

    Unsigned, like every other sync frame, which is why the receiving side
    caps what a peer may assign -- see apply_synced_node_role. This is a
    statement about what THIS node believes, not an instruction.
    """
    try:
        from db_operations import peer_supports
    except Exception:
        return 0
    message = f"ROLE|{str(node_id)}|{str(role)}|{str(updated_at)}"
    sent = 0
    for peer_id in bbs_nodes or []:
